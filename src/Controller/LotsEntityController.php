...
class LotsEntityController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionShow($lots_entity_revision) {
    $lots_entity = $this->entityTypeManager()->getStorage('lots_entity')->loadRevision($lots_entity_revision);
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              'title' => $this->t('Revert'),
              'url' => $has_translations ? Url::fromRoute('entity.lots_entity.translation_revert', [
                'lots_entity' => $lots_entity->id(),
                'lots_entity_revision' => $vid,
                'langcode' => $langcode
